...
8. 租卡前以 `CAPACITY_RPS=0.75`（有据下界：自家 3090 实测 202 tok/s ÷ 平均输出 ~130 tok ≈1.55 req/s 服务率对折；48G 目标机严格更快）运行 `compute_rental_budget.sh`；租卡日用 calibration 真值复算。预算读取 workload 的实际 300/200 默认行数，并与执行端共享 `MIXED_REPEATS`、`OOD_REPEATS`（默认均为 3）以及 7/4 个实际策略档数；另显式计入默认 25 分钟的 `ood_labeling_800_direct_vllm`。超过 7.25 小时硬失败（租期按 8 小时计，45 分钟只保留作重试余量）。用户决策 2026-07-19：预算门槛由 6h 放宽至 8h，保统计规模。
