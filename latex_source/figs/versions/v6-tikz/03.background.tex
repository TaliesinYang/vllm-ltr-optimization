...
token granularity; vLLM's continuous batching and paged KV-cache are the
substrate for this work. The scheduler decides which waiting request joins
...
\begin{figure*}[t]
  ...
  behaviour when scoring exceeds its 15\,ms budget. Shaded components are
...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}[node distance=6mm and 9mm]
    ...
    \node[proposed component, above right=1mm and 11mm of cls, text width=26mm]
    ...
    \node[environment component, below right=1mm and 11mm of cls,
    ...
    \draw[control flow] (cls.east) -- node[fact label, pos=.55, above=.5pt]
      {S3 / S4} (rank.west);
    \draw[data flow] (rank) -- (ord);
    \draw[degraded flow] (cls.east) -- node[warn label, pos=.52, below=1pt,
      align=center] {S1 / S2 / zero-tool\\no Ranker invocation} (queue.west);
...
\emph{Ranking Tau} is Kendall's $\tau_b$ between predicted and true
