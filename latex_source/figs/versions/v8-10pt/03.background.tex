...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}[node distance=5mm and 8mm]
    ...
    \node[environment component, right=of client, text width=27mm] (gw)
    ...
    \node[environment component, right=24mm of dec, text width=19mm] (fb)
      {Fallback\\[-1pt]{\sub{arrival order}}};
    \node[proposed component, below=8mm of dec, text width=25mm] (order)
      {Queue order\\[-1pt]{\sub{policy under test}}};
    \node[environment component, below=8mm of gw, text width=27mm] (engine)
    ...
    % pos=.5 lands this label exactly on the system boundary, and its white
    % fill then erases the rule it sits on. Push it into the clear span.
    \draw[degraded flow] (dec) -- node[warn label, pos=.55, above=1pt]
      {$>15$\,ms} (fb);

    \begin{scope}[on background layer]
      \node[system boundary, fit=(gw)(dec)(order)(engine)] (sys) {};
    \end{scope}
    \node[boundary title, anchor=south west]
      at ($(sys.north west)+(1pt,1.5pt)$) {serving stack under study};
  \end{tikzpicture}
...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}[node distance=5mm and 11mm]
    \node[environment component, text width=26mm] (req)
      {Request\\[-1pt]{\sub{prompt + schema}}};
    \node[proposed component, right=of req, text width=31mm] (cls)
      {Stratum classifier\\[-1pt]{\sub{hash vs vocabulary}}};
    \node[proposed component, above right=2mm and 20mm of cls, text width=33mm]
      (rank) {Ranker\\[-1pt]{\sub{BERT fp16, batch $\leq$ 8}}};
    \node[proposed component, right=of rank, text width=29mm] (ord)
      {Rank order\\[-1pt]{\sub{trusted slots only}}};
    \node[environment component, below right=6.5mm and 20mm of cls,
      text width=33mm] (queue) {Fallback queue\\[-1pt]{\sub{arrival order}}};
    \node[environment component, right=of queue, text width=29mm] (served)
