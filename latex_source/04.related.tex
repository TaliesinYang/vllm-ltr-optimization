...
accuracy will not repair that, so our endpoints are pooled means rather
than tail percentiles. The concession is scoped by the papers' own designs, however:
...
default-cap ordering runs hold it
disabled to isolate ordering, one round enables it to measure it, and the
capped-batch round holds it on in every arm. Above the engine, gateways
