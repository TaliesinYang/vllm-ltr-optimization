...
% Figure inventory for the >=6 rule: fig:workload, fig:tau, fig:coldstart,
% fig:overhead, fig:gate, fig:regime, fig:block1.
\section{Evaluation}

\subsection{E1: What Agent Requests Are Made Of}
Our capture of a production coding agent covers 75 requests across 25 tasks.
...
Fig.~\ref{fig:coldstart} evaluates the same models on the S3 (partially new
tools, $n{=}543$) and S4 (all-new tools, $n{=}333$) strata. Both
...
with no degradation rather than a demonstration of it.
\begin{figure*}[t]\centering
  % Source: runs/offline-experiments-2026-07-25/t1-strata.json
  \includegraphics[width=\linewidth]{coldstart.pdf}
  \caption{Cold-start transfer: schema-text ranking differs by 0.0075
  between the two adequately powered unseen-tool strata, and the identity
  lookup records no table hits. Panel (a) pools all 999 test requests,
  including the withheld S1 and S2 of Fig.~\ref{fig:strata}, so it is not the
  mean of (b) and (c); markers are the mean over three seeds, whiskers are
  session-clustered bootstrap CIs (1000 resamples, percentile, seed-17
  scores), and each $\Delta\tau_b$ interval adds the two arms' variances as
  independent, which is conservative for these paired rows.}
  \label{fig:coldstart}
\end{figure*}
...
p50). Under the documented 15\,ms default,
34.6\%
of requests (801 of 2{,}315) fail open into arrival order, silently; at
50\,ms fail-open drops to 2.0\%, and at 75\,ms it reaches zero.
\begin{figure*}[tb]\centering
  \includegraphics[width=\linewidth]{overhead.pdf}
  \caption{Admission-time scoring versus direct: $+$737\,ms TTFT and
  $+$840\,ms TTLT over the whole added path.}
  \label{fig:overhead}
\end{figure*}
...
not overstate on any stratum here (worst deviation $-0.016$). S2 is where
the Ranker fails: familiar tools in new combinations realize $\tau = 0.44$,
some 0.2 below every other stratum ($n{=}78$).
\begin{figure*}[tb]\centering
...
time-to-last-token, compared as paired-mean ratios with hierarchical
bootstrap intervals against a pre-declared 3\% margin.
...
arrives with the switch of implementation itself. We did not isolate which
internal difference produces it, and none of it is contributed by ordering.
Interposition does not explain it: the shim lands within 2\,ms of stock
(4903 vs.\ 4905\,ms).
...
and with an empty queue the gate itself costs under 1.4\%.
...
steps; 12.4\% of requests first entered a queue of depth two or more.
...
E6 caps it at 16 and repeats the question with everything else fixed, at
6.4 requests/s with prefix caching on and one cold launch per arm. The cap
...
\begin{figure*}[tb]\centering
  ...
  separates, and with the cap reduced every pair does.}
  \label{fig:regime}
\end{figure*}
\begin{figure*}[tb]\centering
