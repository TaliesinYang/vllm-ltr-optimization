...
our budget. The mode's trade is explicit: the GPU never waits for the
scheduler, and in exchange each step is planned on state one step old ---
completions free their slots a step late, arrivals join a step later, and a
token scheduled past a stop is discarded. Timing changes; output never does.
...
caching on and one cold launch per arm. Three conditions besides the cap
differ from round~A --- rate, caching, launch structure --- so the panels
compare regimes, not otherwise-identical runs; held exactly are the
workload, arrival offsets, checkpoint and the four policies, which keeps the
ordering comparison \emph{within} each regime clean. The cap
