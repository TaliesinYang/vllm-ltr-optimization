...
% [数据到位时间] 离线打分完成后；在此之前 fig4.pdf 必须保持缺失。
...
% [作者手写区] 要点: Fig.5 的 Tier-2 held-out test tau-b 统一采用三 seed 均值: prompt-only/prompt+schema/full-context = 0.587/0.630/0.626；误差线是三 seed 极差。LightGBM 仅 seed 42，test tau-b=0.426799；不得与 validation 0.446937 混写。
% [作者手写区] 要点: 同图读取 \texttt{tier2-learning-curve.json}；full-context seed42 在 500/1000/2000/4000 训练池的 validation tau-b=0.605171/0.610124/0.637282/0.632051；强调 4000 未继续上升，而不是挑单点。Bib keys: bert, lightgbm, pars2025.
...
% [数据到位时间] 离线打分完成后；在此之前 fig6.pdf 必须保持缺失。
...
% [数据到位时间] 离线打分完成后；在此之前 fig7.pdf 必须保持缺失。
...
% [数据到位时间] 租卡日；仅在 matrix/parity/run manifest 全部归档后生成 fig8.pdf。
