...
\begin{table}[t]
  ...
  3{,}997/998/999 split of the ToolACE tier-2 sample.}
  \label{tab:variants}
...
(S1$=$45, S2$=$78) are withheld from headline claims; they may inform design
when disclosed with their size. The Reliability Gate assigns each stratum the
...
measured our scorer would actually set. vLLM prefix caching is disabled uniformly across all
ordering arms, verified from the engine's reported hit rate rather
than from the flag alone, and enabled only in the arm that measures
its effect.
...
disabled in every ordering arm --- verified from the engine's own reported
hit rate rather than from the flag alone --- and enabled only in the round
that measures it. The scheduler is
replaced through vLLM's~\cite{vllm2023} \texttt{--scheduler-cls} seam; each
policy arm swaps only its ordering rule, while the stock pair differs in
