...
did. With vLLM's running-batch cap at its default of 256 slots, mean
occupancy stayed under a thirteenth of it, the waiting queue was empty at the
90th percentile, and no ordering policy separated from another by more than
1.2\%. Capping the batch at sixteen puts 57--64\% of requests behind another,
...
gate is the part that does not pay: abstained requests hold their arrival
slots and block the ones behind them, which costs 9.5\% against the ungated
ranker and leaves the gated system 4.9\% slower than plain arrival order
(1.049, $[1.033, 1.065]$). The
